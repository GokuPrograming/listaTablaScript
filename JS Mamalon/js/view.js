...
export default class View {
  ...
  filter(filters) {
    ...
    for (const row of rows) {
      ...
      if (words) {
        shouldHide = !title.innerText.includes(words) && !description.innerText.includes(words) && !date.innerText.includes(words);
      }
  ...
  editTodo(id, values) {
    const fech = new Date(values.date);
    ...
    row.children[2].innerText = fechaActualizada.toLocaleDateString();
